...
!function () {
    ...
    var TJPFloatEvent = {
        ...
        open: function () {
            ...
            if (!self.running) {
                self.running = true;
                l("#appd_wrap_open_cnt").css({ left: "-100%" }).parent().hide();
                l("#fl_pop_wrap_cntr").css({ left: "0" }).parent().show();
                self.running = false;
            }
        },
        close: function () {
            ...
            if (!self.running) {
                self.running = true;
                l("#fl_pop_wrap_cntr").css({ left: "-100%" }).parent().hide();
                l("#appd_wrap_open_cnt").css({ left: "0" }).parent().show();
                self.running = false;
            }
        },
        allClose: function () {
            ...
            if (!self.running) {
                self.running = true;
                l("#fl_pop_wrap_cntr").parent().hide();
                l("#appd_wrap_open_cnt").parent().hide();
                self.running = false;
            }
        },
        addTrack: function () {
            if (typeof (_gaq) !== "undefined" && _gaq) {
                var tjpageId = l("#tjpage_id").val();
                if(tjpageId>0)
                {
                    utility.addEventHandler(document.getElementById("appd_wrap_close"), "click", function () {
                        _gaq.push(['_trackEvent', "PC-浮层广告关闭", tjpageId, ""]);
                    });

                }
            }
            else {
                // Log ga not active issue, if offen accurs , add alternate way to handle this, usually this  is enough;
            }

        },
        changeDowloadErweima: function () {
            var tjpageId = l("#tjpage_id").val();
            var isSeo = window.location.href.indexOf("se0") >= 0 || window.location.href.indexOf("utm_source") >= 0;
            var QRcodeTimer = setInterval(function () {
                // 创建二维码方法存在，二维码容器存在，二维码还未创建
                if (window.createQRcodeInFloatLayer && $('#popfloating_float_img').length) {
                    window.createQRcodeInFloatLayer();
                    clearInterval(QRcodeTimer);
                } else {
                    return;
                }
                // console.log('QRcodeTimer');
            }, 2000);
        }
    };
    ...
    window.TJPFloating = {
        BuildHTML: function (data) {
            function registerToggleEvent() {
                l("#appd_wrap_open_cnt").on("click", function () { TJPFloatEvent.expand(); });
                l("#appd_wrap_close").on("click", function () {
                    if (manualClosed!="1") {
                        utility.setCookie({ manualclose: "1" }, "", 7);
                    }
                    TJPFloatEvent.collapse();
                });
            }
            ...
            /* 浮动内容为空 */
            if ("" == data.FloatContent || void 0 == data.FloatContent) {
                // TODO: Log float content null log
                return;
            }

            var manualClosed = utility.getCookie("manualclose");
            var decodeContent = decodeURIComponent(data.FloatContent.replace((/\\/g), ""));
            var domBody = document.getElementsByTagName("body")[0];
            domBody.insertBefore(utility.html2dom(decodeContent), null);
            registerToggleEvent();
            TJPFloatEvent.addTrack();
            // 根据页面不周展示不同下载二维码
            TJPFloatEvent.changeDowloadErweima();


            var defaultCollapseDelay = 20000;
            var mouseOutCollapseDelay = 20000;

            if (manualClosed == 1 || data.DefaultClose)
            {
                TJPFloatEvent.close();
            }
            else {
            ...
            var collapseTimer = setTimeout(function () { TJPFloatEvent.collapse() }, defaultCollapseDelay);
            var defaultFloat = document.getElementById("appd_wrap_default");
            utility.addEventHandler(defaultFloat, "mouseover", function (e) {
                if (utility.checkHover(e, this) || navigator.userAgent.indexOf("MSIE") > 0)
                    clearTimeout(collapseTimer);
            });
            utility.addEventHandler(defaultFloat, "mouseout", function (e) {
                if (utility.checkHover(e, this) || navigator.userAgent.indexOf("MSIE") > 0)
                    collapseTimer = setTimeout(function () { TJPFloatEvent.collapse() }, mouseOutCollapseDelay);
            });
        },
        getPopHTML: function () {
